...
// volume records that they moved data. The RIGHT place to stop them
// reaching the federation is a never-publish rule in internal/publish,
// which would drop them at the seal without breaking the copy or the
// delete that created them; that is written down as a follow-up in
// docs/TODO.md and is deliberately not built here, because a filter in
// this file cannot tell a Finder copy from a user's own file.
