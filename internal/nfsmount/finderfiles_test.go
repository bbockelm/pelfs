...
// Every name on the list, through the one method a Finder actually reaches
// first. A list with a typo in it -- ".Spotlight_V100", ".DSStore" -- would
// otherwise pass every test above and publish the file it was meant to
// stop.
func TestEveryNameOnTheListIsRefused(t *testing.T) {
	fs := filtered()
	for _, name := range []string{
		".DS_Store", ".Spotlight-V100", ".fseventsd",
		".DocumentRevisions-V100", ".VolumeIcon.icns", ".apdisk",
	} {
		if _, err := fs.Create("/" + name); !errors.Is(err, os.ErrPermission) {
			t.Errorf("Create(%s) = %v, want a permission error", name, err)
		}
		if _, err := fs.Stat("/" + name); !errors.Is(err, os.ErrNotExist) {
			t.Errorf("Stat(%s) = %v, want not-exist", name, err)
		}
...
// user asked for rather than housekeeping they did not. The right place to
// keep them out of the FEDERATION is a never-publish rule at the seal
// (docs/TODO.md), which does not have to break the copy to do it.
...
// The filter follows a Chroot, and it has to match on the last component
// for that to be true: go-nfs chroots the served filesystem, so a rule
// written against a full path would stop matching the moment the root
// moved.
func TestTheFilterSurvivesAChroot(t *testing.T) {
	inner := memfs.New()
	if err := inner.MkdirAll("/sub", 0755); err != nil {
		t.Fatal(err)
	}
	sub, err := diagnose(inner, finderDropping).Chroot("/sub")
	if err != nil {
		t.Fatalf("Chroot: %v", err)
	}
	if _, err := sub.Create("/.DS_Store"); !errors.Is(err, os.ErrPermission) {
		t.Errorf("Create(.DS_Store) inside a chroot = %v, want a permission error", err)
	}
	if _, err := sub.Create("/keep.txt"); err != nil {
		t.Errorf("Create(keep.txt) inside a chroot = %v, want success", err)
	}
}
