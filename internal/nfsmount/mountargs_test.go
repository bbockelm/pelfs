...
// A NAME without --finder still names the export and still hides the
// volume. The two options are independent by construction, and asserting
// it is what keeps a later "simplification" from folding them together:
// the export name is a label the client may synthesize a volume name from,
// and nobrowse is what decides whether anything shows it.
func TestMountCommandNameWithoutBrowsableStillHides(t *testing.T) {
	_, args := mountCommand("darwin", 2049, "/Users/x/mnt", MountOptions{VolumeName: "Data"})
	if o := opts(t, args); !has(o, "nobrowse") {
		t.Errorf("a named but non-browsable mount lost nobrowse: %q", o)
	}
