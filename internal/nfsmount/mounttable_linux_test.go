...
// rows that make the parser more than a Fields call: an NFS mount whose
// source carries the server and export, and a mountpoint with a space in
// it, which the kernel writes as \040 and which is exactly the shape a
// Finder-style volume name produces.
