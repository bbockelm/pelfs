...
// The watch fires ONCE and then stops. What the caller does on the fire is
// run a teardown, and a second fire has nowhere to go: it would either
// close an already-closed channel (a panic) or re-enter a teardown that is
// mid-seal.
func TestWatchUnmountFiresOnlyOnceAndStops(t *testing.T) {
	tick := make(chan time.Time, 4)
	gone, stopped := watchUnmount(context.Background(), func() (bool, error) { return false, nil }, tick)
	for i := 0; i < 4; i++ {
		tick <- time.Now()
	}
	<-gone
	<-stopped
	// Every tick after the first is left in the buffer, which is the
	// evidence that the loop returned rather than reading them: a loop
	// still running would have drained the channel and closed gone twice.
	if len(tick) == 0 {
		t.Error("the watch kept consuming ticks after it fired")
	}
}
...
// WatchUnmount is the exported wrapper, and the thing worth asserting
// about it is that it stops: a session that ends for its own reasons must
// not leave a goroutine polling getfsstat(2) forever, and this is the only
// caller-facing entry point.
func TestWatchUnmountCancelsCleanly(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	gone := WatchUnmount(ctx, "/pelfs-no-such-mountpoint-9c1f")
	cancel()
	select {
	case <-gone:
		t.Error("a cancelled WatchUnmount reported an unmount")
	case <-time.After(50 * time.Millisecond):
		// The poll interval is two seconds, so nothing should have
		// happened either way; what this asserts is that cancelling does
		// not itself close the channel.
	}
}
