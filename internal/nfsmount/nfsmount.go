...
// ServeOptions is how one server differs from the default. The zero value
// is the server pelfs has always run.
...
func (s *Server) Mount(mountPoint string, opts MountOptions) error {
	...
	// WINDOWS IS REFUSED HERE, not left to fail as an exec. mountCommand's
	// default is macOS's mount_nfs and its only override is Linux's
	// mount(8), so a Windows build used to fall through to `mount_nfs` and
	// report `executable file not found in %PATH%` — an error about a
	// program the user never asked for. Windows does have an NFS client
	// (Client for NFS, `mount.exe`), but it is an optional feature, it
	// speaks a different option vocabulary, and it has never been
	// exercised against this server; claiming support by guessing at its
	// flags would be worse than saying so.
...
// even though its reason for existing is macOS without macFUSE. Windows
// never reaches this function; Mount refuses it above, by name.
func mountCommand(goos string, port int, mountPoint string, opts MountOptions) (string, []string) {
	...
	if !opts.Browsable {
		o = append(o, "nobrowse") // keep the volume out of the macOS GUI
	}
...
//
// A mount table that cannot be READ leaves the escalation to run exactly
// as it did before: Mounted's error means "unknown", never "not mounted".
