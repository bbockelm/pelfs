...
// (see parseSecurityPassword) to deserve tests that run in the Linux and
// Windows CI lanes as well as the macOS one.
...
// user looking for this item would look. THAT is why every test that runs
// this must redirect $HOME and assert its own sandbox keychain is the
// default first: getting it wrong writes into the user's login keychain,
