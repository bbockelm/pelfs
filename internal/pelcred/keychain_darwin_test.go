...
//
// It never touches the user's login keychain. The Security framework
// resolves both the default keychain and the search list through $HOME, so
// the test redirects HOME to a scratch directory and creates a keychain
// there; `security` inherits the redirected environment, and the login
// keychain is not reachable from inside the test.
...
func sandboxKeychain(t *testing.T) string {
	...
	if got := run("default-keychain"); !strings.Contains(got, kcPath) {
