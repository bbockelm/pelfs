...
// macOS Keychain, so opening a volume stops asking for it.
//
...
// is the kernel session keyring (config/keyring_default.go, built for
// `!linux || (linux && !amd64)`). macOS therefore gets the weakest version:
// the password is remembered until the process exits and not one moment
// longer, so every `pelfs shell`, every `pelfs mount`, every `pelfs browse`,
// every `pelfs gc` asks again.
//
...
//
// # Why this is the prompt that had to go, and not merely a comfort
//
// pelfs has two federation prompts and they are not alike. The device-flow
// prompt (a URL and a code) needs no terminal — pelican has no IsTerminal
// gate on it, which is what lets `pelfs browse` render it as a card on the
// page (cmd/pelfs/ssoprompt.go). The WALLET prompt does: config.GetPassword
// refuses outright when stdin is not a character device, and when it is, it
// puts that terminal into raw mode.
//
// And it comes FIRST. client.AcquireToken opens the wallet
// (GetCredentialConfigContents) before it can know an OAuth client is
// needed, so on macOS a locked wallet stops a `pelfs browse` in the
// terminal the user was just told they would not need — before the SSO card
// it would have shown is reachable at all. Seeding the password from the
// Keychain is what makes that promise true rather than aspirational.
//
