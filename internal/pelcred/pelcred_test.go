...
func TestStalePasswordWithNoTerminalDoesNotLoop(t *testing.T) {
	// typed == nil: nobody is there to answer Pelican's prompt. This is
	// `pelfs mount`'s detached child, and it is why the stale value is
	// dropped rather than retried — a retry would spend an authorization
	// prompt to fail in exactly the same way.
...
func TestParseSecurityPassword(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Windows-style line endings cannot come from security, but the
		// parser trims them and a regression here would be silent.
		{"carriage return", "password: \"hunter2\"\r\n", "hunter2", true},
...
func TestLookupAndSaveArguments(t *testing.T) {
	...
	//   - -w is the FINAL argument and has NOTHING after it, which is what
	//     makes security prompt for the password rather than take it from
	//     the command line — and is the rule whose violation once wrote a
	//     stray item into a real login keychain, because -w swallowed the
	//     positional that followed it;
	...
	// The same rule stated as the thing that went wrong: -w must not have a
	// positional after it to swallow, in the add or in any other command.
	for _, c := range calls {
		for i, a := range c.args {
			if a == "-w" && i != len(c.args)-1 {
				t.Errorf("%v puts %q after -w, which security would read as the password",
					c.args, c.args[i+1])
			}
		}
	}
...
// present makes this process look like one with a person at its terminal,
// so that each refusal below is attributable to its own clause rather than
// to the test binary's stdin never being a tty (or, in the Linux lane, to
// the test itself running inside a container).
func present(t *testing.T) {
	t.Helper()
	saveTTY, saveEuid, saveContainer := isTerminal, geteuid, inContainer
	isTerminal = func(*os.File) bool { return true }
	geteuid = func() int { return 501 }
	inContainer = func() bool { return false }
	t.Cleanup(func() { isTerminal, geteuid, inContainer = saveTTY, saveEuid, saveContainer })
	for _, k := range []string{"CI", "GITHUB_ACTIONS", "CONTINUOUS_INTEGRATION", "BUILD_NUMBER"} {
		t.Setenv(k, "")
	}
}

func TestInteractiveRefusesEveryHeadlessCase(t *testing.T) {
	// The positive clause first, and it runs in every lane: without it the
	// refusals below would all pass trivially in a test binary whose stdin
	// is never a terminal.
	t.Run("a person at a terminal", func(t *testing.T) {
		present(t)
		if !interactive() {
			t.Error("refused to ask a question of a person at a terminal")
		}
	})
	...
	for _, tc := range []struct {
		...
		// `pelfs mount`'s detached child: stdin is /dev/null and stderr is
		// the log file. Either one alone is enough to disqualify it.
		{"only stdin is a terminal", func(t *testing.T) {
			save := isTerminal
			isTerminal = func(f *os.File) bool { return f == os.Stdin }
			t.Cleanup(func() { isTerminal = save })
		}},
		{"only stderr is a terminal", func(t *testing.T) {
			save := isTerminal
			isTerminal = func(f *os.File) bool { return f == os.Stderr }
			t.Cleanup(func() { isTerminal = save })
		}},
		...
		{"Jenkins", func(t *testing.T) { t.Setenv("BUILD_NUMBER", "417") }},
		{"in a container", func(t *testing.T) {
			save := inContainer
			inContainer = func() bool { return true }
			t.Cleanup(func() { inContainer = save })
		}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			present(t)
			tc.set(t)
			if interactive() {
				t.Errorf("would ask a question with %s", tc.name)
			}
		})
	}

	// An SSH session is deliberately NOT a refusal: the password question
	// is answerable there, and the Keychain it would be stored in is the
	// remote machine's, which is the one the next pelfs on that machine
	// reads.
	t.Run("over SSH", func(t *testing.T) {
		present(t)
		t.Setenv("SSH_CONNECTION", "10.0.0.1 22 10.0.0.2 22")
		if !interactive() {
			t.Error("refused a question over SSH, where it is perfectly answerable")
		}
	})
}

func TestTerminalAskerNeedsAnExplicitYes(t *testing.T) {
	// The default matters more than the parsing: this question is only ever
	// asked about writing a secret somewhere new, so everything that is not
	// an explicit yes — including end of input — is a no.
	for _, tc := range []struct {
		typed string
		want  bool
	}{
		...
		{"\n", false},
		{"yeah\n", false},
		{"", false}, // EOF: the pipe closed without a line
	} {
		t.Run("answers "+tc.typed, func(t *testing.T) {
			present(t)
			restore := feedStdin(t, tc.typed)
			defer restore()
			var ask terminalAsker
			if got := ask.Confirm("store it? "); got != tc.want {
				t.Errorf("Confirm with %q = %v, want %v", tc.typed, got, tc.want)
			}
		})
	}
}

func TestTerminalAskerRefusesWhenNobodyIsThere(t *testing.T) {
	// Even with a "y" waiting to be read. A background mount must not be
	// able to answer this question on the user's behalf.
	save := isTerminal
	isTerminal = func(*os.File) bool { return false }
	t.Cleanup(func() { isTerminal = save })
	...
	defer restore()
	var ask terminalAsker
	if ask.Confirm("store it? ") {
		t.Error("stored a secret on the word of a process with no terminal")
	}
}

// feedStdin points os.Stdin at a pipe holding what the user "typed" and
// os.Stderr at a discard, so the prompt does not land in the test log. It
// returns the restore rather than using t.Cleanup so the descriptors are
// back before the subtest reports.
func feedStdin(t *testing.T, typed string) func() {
	t.Helper()
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	...
	// Closed immediately: Confirm reads one line and must see EOF rather
	// than block when the answer is empty.
