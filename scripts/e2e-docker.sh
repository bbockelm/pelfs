...
"$BIN"/pelfs shell --docker --snapshot-interval 0 --writeback --flush-timeout 5m \
...
"$BIN"/pelfs shell --docker --snapshot-interval 0 --prefetch all --stats-file "$WORK/stats-pf.json" "$PREFIX" > "$WORK/run-pf.log" 2>&1 <<'EOF'
cat hello.txt
EOF
...
{"session":"other-client","hostname":"elsewhere","pid":4242,
 ...
 "ttl_seconds":600}
LEASE
if echo exit | "$BIN"/pelfs shell --docker --snapshot-interval 0 "$PREFIX" > "$WORK/run-lease.log" 2>&1; then
...
echo exit | "$BIN"/pelfs shell --docker --snapshot-interval 0 --steal-lease "$PREFIX" > "$WORK/run-steal.log" 2>&1 \
